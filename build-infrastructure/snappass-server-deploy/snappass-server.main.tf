...
data "local_file" "image_version" {
  filename = "${ path.root }/snappass-server-image/builds/latest-version"
}
...
resource "libvirt_volume" "root" {
  ...
  source = "${ path.root }/snappass-server-image/builds/${ local.image_version }/snappass-server.qcow2"
  format = "qcow2"
...
resource "libvirt_cloudinit_disk" "cloud_init" {
  ...
  network_config = templatefile("${ path.module }/snappass-server.network-config.tftpl", {
  ...
  user_data  = templatefile("${ path.module }/snappass-server.user-data.tftpl", {
    hostname = var.hostname
    domain   = var.domain
