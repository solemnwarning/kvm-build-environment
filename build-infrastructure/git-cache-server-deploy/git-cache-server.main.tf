...
data "local_file" "image_version" {
  filename = "${ path.root }/git-cache-server-image/builds/latest-version"
}
...
resource "libvirt_volume" "root" {
  ...
  source = "${ path.root }/git-cache-server-image/builds/${ local.image_version }/git-cache-server.qcow2"
  format = "qcow2"
...
resource "libvirt_cloudinit_disk" "cloud_init" {
  ...
  network_config = templatefile("${ path.module }/git-cache-server.network-config.tftpl", {
  ...
  user_data  = templatefile("${ path.module }/git-cache-server.user-data.tftpl", {
    hostname = var.hostname
    domain   = var.domain
