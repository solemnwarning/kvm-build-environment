...
locals {
  ...
  ovmf_path     = "${ path.root }/macos1013-build-agent-image/builds/${ local.image_version }/OVMF_CODE.fd"
  vars_path     = "${ path.root }/macos1013-build-agent-image/builds/${ local.image_version }/OVMF_VARS.fd"

  output_image_name = "macos1013-build-agent-${ local.image_version }.qcow2"
}
...
resource "local_file" "domain-xml" {
  content  = templatefile("${ path.module }/macos1013-build-agent.xml.tftpl", {
  ...
  filename = "${ var.template_dir }/macos1013-build-agent.xml"
}
...
resource "local_file" "user-data" {
  content  = templatefile("${ path.module }/macos1013-build-agent.user-data.tftpl", {
    domain = "${ var.domain }"
