...
data "local_file" "image_version" {
  filename = "${ path.root }/copr-cli-agent-image/builds/latest-version"
}

locals {
  image_version = chomp(data.local_file.image_version.content)
  image_path    = "${ path.root }/copr-cli-agent-image/builds/${ local.image_version }/copr-cli-agent.qcow2"
}
...
resource "libvirt_cloudinit_disk" "cloud_init" {
  ...
  network_config = templatefile("${ path.module }/copr-cli-agent.network-config.tftpl", {})
