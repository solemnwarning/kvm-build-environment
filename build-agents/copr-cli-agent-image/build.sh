...
if [ -n "$quiet" ]
then
	packer build -timestamp-ui -var "output_dir=builds/${timestamp}" copr-cli-agent.pkr.hcl > "${log}" 2>&1 \
		|| (status=$?; cat "${log}"; rm -f "${log}"; exit $status)
else
	packer build -timestamp-ui -var "output_dir=builds/${timestamp}" copr-cli-agent.pkr.hcl |& tee "${log}"
fi
